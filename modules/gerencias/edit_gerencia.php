<?php
/**
 * modules/gerencias/edit_gerencia.php
 *
 * Página para editar una Gerencia existente.
 * Carga los datos de la gerencia por su ID y permite modificarlos.
...
if (!in_array('Administrador', $_SESSION['perfiles'])) {
    $_SESSION['message'] = ['type' => 'error', 'text' => 'Acceso denegado. No tiene permisos para editar gerencias.'];
...
$id_gerencia = $codigo_gerencia = $nombre_gerencia = "";
$activo = 0; // Se inicializa en 0 para el checkbox
$codigo_gerencia_err = $nombre_gerencia_err = "";

// Procesa el formulario cuando se envía (POST request)
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Validar y sanear la entrada del ID de gerencia
    $id_gerencia = filter_input(INPUT_POST, 'id_gerencia', FILTER_VALIDATE_INT);
    if (!$id_gerencia) {
        $_SESSION['message'] = ['type' => 'error', 'text' => 'ID de gerencia inválido.'];
    ...
    if (empty(trim($_POST["codigo_gerencia"]))) {
        ...
        // Preparar una sentencia SELECT para verificar si el código ya existe para otro ID
        $sql_check_codigo = "SELECT id_gerencia FROM gerencias WHERE codigo_gerencia = ? AND id_gerencia != ?";
        if ($stmt = $conn->prepare($sql_check_codigo)) {
            $stmt->bind_param("si", $param_codigo_gerencia, $param_id_gerencia_check);
            $param_codigo_gerencia = $codigo_gerencia_input;
            $param_id_gerencia_check = $id_gerencia;

            if ($stmt->execute()) {
                $stmt->store_result();
                if ($stmt->num_rows == 1) {
                    $codigo_gerencia_err = "Este código de gerencia ya está registrado por otra gerencia.";
                } else {
    ...
    if (empty(trim($_POST["nombre_gerencia"]))) {
        ...
        // Preparar una sentencia SELECT para verificar si el nombre ya existe para otro ID
        $sql_check_nombre = "SELECT id_gerencia FROM gerencias WHERE nombre_gerencia = ? AND id_gerencia != ?";
        if ($stmt = $conn->prepare($sql_check_nombre)) {
            $stmt->bind_param("si", $param_nombre_gerencia, $param_id_gerencia_check);
    ...
    // Si no hay errores de entrada, actualizar la gerencia en la base de datos
    if (empty($codigo_gerencia_err) && empty($nombre_gerencia_err)) {
        // La tabla tiene 'codigo_gerencia', 'nombre_gerencia', 'activo'
        $sql_update = "UPDATE gerencias SET codigo_gerencia = ?, nombre_gerencia = ?, activo = ? WHERE id_gerencia = ?";

        if ($stmt = $conn->prepare($sql_update)) {
            $stmt->bind_param("ssii", $param_codigo_gerencia, $param_nombre_gerencia, $param_activo, $param_id_gerencia);
            $param_codigo_gerencia = $codigo_gerencia;
            $param_nombre_gerencia = $nombre_gerencia;
            $param_activo = $activo;
            $param_id_gerencia = $id_gerencia;

            if ($stmt->execute()) {
                $_SESSION['message'] = ['type' => 'success', 'text' => 'Gerencia "' . htmlspecialchars($nombre_gerencia) . '" (Código: ' . htmlspecialchars($codigo_gerencia) . ') actualizada exitosamente.'];
                header("location: index.php");
                exit();
            } else {
                $_SESSION['message'] = ['type' => 'error', 'text' => 'Error al actualizar la gerencia: ' . $stmt->error];
            }
            $stmt->close();
        } else {
    ...
    $conn->close();
} else { // Si es un GET request, cargar los datos de la gerencia
    if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
        $id_gerencia = filter_var(trim($_GET["id"]), FILTER_VALIDATE_INT);

        if ($id_gerencia === false) {
            $_SESSION['message'] = ['type' => 'error', 'text' => 'ID de gerencia no válido.'];
            header("location: index.php");
            exit();
        }

        // Prepara la sentencia SELECT para obtener los datos de la gerencia
        // Seleccionamos 'codigo_gerencia' y 'activo'
        $sql_select = "SELECT id_gerencia, codigo_gerencia, nombre_gerencia, activo FROM gerencias WHERE id_gerencia = ?";
        if ($stmt_select = $conn->prepare($sql_select)) {
            $stmt_select->bind_param("i", $param_id);
            $param_id = $id_gerencia;

            if ($stmt_select->execute()) {
                $result = $stmt_select->get_result();
                if ($result->num_rows == 1) {
                    $row = $result->fetch_assoc();
                    $codigo_gerencia = $row['codigo_gerencia'];
                    $nombre_gerencia = $row['nombre_gerencia'];
                    $activo = $row['activo']; // Carga el estado activo
                } else {
                    $_SESSION['message'] = ['type' => 'error', 'text' => 'No se encontró la gerencia.'];
                    header("location: index.php");
                    exit();
                }
            } else {
                $_SESSION['message'] = ['type' => 'error', 'text' => '¡Ups! Algo salió mal al cargar la gerencia.'];
                header("location: index.php");
                exit();
            }
            $stmt_select->close();
        ...
        $_SESSION['message'] = ['type' => 'error', 'text' => 'Parámetro de ID de gerencia no proporcionado.'];
        header("location: index.php");
        exit();
    }
    $conn->close();
...
<body>
    ...
    <div class="main-content">
        ...
        <p>Modifique los datos de la gerencia.</p>

        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="data-form">
            <input type="hidden" name="id_gerencia" value="<?php echo htmlspecialchars($id_gerencia); ?>">
